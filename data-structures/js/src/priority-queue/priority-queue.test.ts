...
describe("Priority Queue", () => {
    ...
    describe("Custom Comparator Priority Queue", () => {
      ...
      test("Basic Functionality", () => {
        ...
        for (let i = 0; i < peopleSize; i++) {
          ...
          expect(oldestPersonInQueue?.name).toBe(
            peopleSortedByAgeDesc[i]?.name
          );
  ...
  describe("Start With A List", () => {
    describe("Min Priority Queue", () => {
      let pqueue: PriorityQueue<number>;
      const arr: number[] = [4, 7, 9, 2, 5, 1];
      const expectedArr: number[] = [1, 2, 4, 5, 7, 9];
      const size = arr.length;
      beforeEach(() => {
        pqueue = new PriorityQueue<number>({ list: arr });
      });

      test("Basic Functionality", () => {
        expect(pqueue.size).toBe(size);
        expect(pqueue.top).toBe(expectedArr[0]);
        for (let i = 0; i < size; i++) {
          expect(pqueue.remove()).toBe(expectedArr[i]);
        }
      });
    });
    ...
    describe("Max Priority Queue", () => {
      let pqueue: PriorityQueue<number>;
      const arr: number[] = [4, 7, 9, 2, 5, 1];
      const expectedArr: number[] = [1, 2, 4, 5, 7, 9].reverse();
      const size = arr.length;
      beforeEach(() => {
        pqueue = new PriorityQueue<number>({ maxHeap: true, list: arr });
      });
